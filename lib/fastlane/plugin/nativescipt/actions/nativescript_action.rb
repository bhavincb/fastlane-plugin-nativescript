module Fastlane
  module Actions
    module SharedValues
      TNS_IOS_RELEASE_BUILD_PATH = :TNS_IOS_RELEASE_BUILD_PATH
      TNS_ANDROID_RELEASE_BUILD_PATH = :TNS_ANDROID_RELEASE_BUILD_PATH
    end

    class NativesciptAction < Action
      # valid action params
      ...
      def self.check_platform(params)
        ...
        if platform && !File.directory?("./platforms/#{platform}")
          if params[:tns_no_fetch]
            sh "tns platform add #{platform} --nofetch"
          else
            sh "tns platform add #{platform}"
          end
        end
      end

      # app_name(have to think or search)
      ...
      def self.build(params)
        ...
        if params[:tns_prepare]
          # TODO: Remove params not allowed/used for `prepare`
          sh "tns prepare #{params[:platform]} #{args.join(' ')}"
        end
      ...
      def self.set_build_paths(is_release)
        ...
        ENV['TNS_ANDROID_RELEASE_BUILD_PATH'] = "./platforms/android/app/build/outputs/apk/#{build_type}/app-#{build_type}.apk"
        ENV['TNS_IOS_RELEASE_BUILD_PATH'] = "./platforms/ios/build/device/#{app_name}.ipa"
      ...
      def self.description
        "Build your Narivescipt app"
      end

      def self.details
        "Easily integrate your Narivescipt build into a Fastlane setup"
      end
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(
            ...
            env_name: "TNS_IOS_PROVISIONING_PROFILE",
          ...
          FastlaneCore::ConfigItem.new(
            key: :keystore_path,
            env_name: "TNS_ANDROID_KEYSTORE_PATH",
          ...
          FastlaneCore::ConfigItem.new(
            ...
            env_name: "TNS_ANDROID_KEYSTORE_PASSWORD",
          ...
          FastlaneCore::ConfigItem.new(
            key: :key_password,
            env_name: "TNS_ANDROID_KEY_PASSWORD",
          ...
          FastlaneCore::ConfigItem.new(
            key: :keystore_alias,
            env_name: "TNS_ANDROID_KEYSTORE_ALIAS",
          ...
          FastlaneCore::ConfigItem.new(
            key: :tns_prepare,
            env_name: "TNS_PREPARE",
            description: "Specifies whether to run `tns prepare` before building",
            default_value: true,
            is_string: false
          ),
          FastlaneCore::ConfigItem.new(
            key: :min_sdk_version,
            env_name: "TNS_ANDROID_MIN_SDK_VERSION",
      ...
      def self.output
        [
          ['TNS_ANDROID_RELEASE_BUILD_PATH', 'Path to the signed release APK if it was generated'],
          ['TNS_IOS_RELEASE_BUILD_PATH', 'Path to the signed release IPA if it was generated']
        ]
      end
